...
ENV SPEEDTEST_CLI_VERSION="1.2.0"
...
RUN --mount=type=bind,from=builder,source=/usr/bin/envsubst,target=/usr/bin/envsubst \
    ...
    && wget -q "https://install.speedtest.net/app/cli/ookla-speedtest-${SPEEDTEST_CLI_VERSION}-linux-x86_64.tgz" -O /speedtest.tar.gz \
    && tar xzvf /speedtest.tar.gz -C /usr/local/bin \
...
HEALTHCHECK CMD nc -vz localhost "${SPEEDTEST_EXPORTER_PORT}" || exit 1 # nosemgrep
